...
public class BookResource {
	...
	@Produces(MediaType.APPLICATION_JSON)
	@Consumes(MediaType.APPLICATION_JSON)
	public BookDto updateBookByIsbn(@PathParam("isbnKey") IntParam isbnKey, @QueryParam("status") Optional<String> title)
	{
		String tempName="";
		int tempId=0;
		for(BookDetails iter: bookMap.values())
		{
			...
			{
				iter.setTitle(title.get());
				tempName=iter.getTitle();
				tempId=iter.getisbnKey();	
			}
		}
		BookDetails bookDisp=new BookDetails();
		bookDisp.setTitle(tempName);
	...
	@GET
	@Path("books/{isbnKey}/reviews/{reviewId}")
